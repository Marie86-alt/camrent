...
export function OwnerProfileScreen({}: Props) {
  ...
  const confirmDeleteAccount = () => {
    if (!user) return;
    bottomSheet.show({
      title: t('owner.delete_title'),
      subtitle: t('owner.delete_confirm'),
      actions: [
        {
          label: t('owner.delete_account'),
          variant: 'danger',
          icon: 'trash-outline',
          onPress: async () => {
            try {
              ...
              hapticError();
              toast.error(t('owner.delete_error'));
            }
          },
        },
      ],
  ...
  return (
    <Screen topSafeArea>
      <View className="gap-5 px-5 pt-4">
        <ProfilePhotoPicker roleLabel={t('owner.role')} user={user} />
        ...
        >
          ...
          <InfoRow icon="call-outline" label={t('auth.phone')} value={user?.phone} />
          <InfoRow icon="location-outline" label={t('common.city')} value={user?.city} last />
        </View>
        ...
        >
          ...
          <Text className="font-semibold text-slate-700">{t('auth.logout')}</Text>
        </TouchableOpacity>
        ...
        >
          ...
          <Text className="font-semibold text-red-700">{t('owner.delete_account')}</Text>
        </TouchableOpacity>
      </View>
    </Screen>
  );
