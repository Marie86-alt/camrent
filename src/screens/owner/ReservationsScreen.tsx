...
export function ReservationsScreen() {
  ...
  const setStatus = useCallback(
    async (booking: Booking, status: Extract<BookingStatus, 'cancelled' | 'confirmed'>) => {
      try {
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="mt-20 items-center gap-3">
              ...
              <Text className="text-base font-bold text-slate-700">{error ?? 'Aucune réservation reçue'}</Text>
            </View>
          }
          data={bookings}
